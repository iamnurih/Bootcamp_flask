...
board_blp = Blueprint('Board', 'board',
...
class BoardList(MethodView):
    def get(self):
        ...
        return jsonify([{"user_id": board.user_id,
                         ...
                         "author": board.author.name} for board in boards])
    ...
    def post(self):
        ...
        db.session.commit()
        return jsonify({"msg": "Board successfully created"}), 201
...
class BoardResource(MethodView):
    ...
    def put(self, board_id):
        ...
        db.session.commit()
        return jsonify({"msg": "Board successfully updated"}), 201

    def delete(self, board_id):
        board = Board.query.get_or_404(board_id)
        db.session.delete(board)
        db.session.commit()
        return jsonify({"msg": "Board deleted successfully"}), 201